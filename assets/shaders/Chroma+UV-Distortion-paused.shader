...
float4 mainImage(VertData v_in) : TARGET
{
    ...
    uvtR += float2(sin(uvt.y*amplitude)/distortion, cos(uvt.x*amplitude)/distortion);
    uvtG += float2(sin(uvt.y*amplitude+chroma)/distortion, cos(uvt.x*amplitude+chroma)/distortion);
    uvtB += float2(sin(uvt.y*amplitude+(chroma*2.))/distortion, cos(uvt.x*amplitude+(chroma*2.))/distortion);
